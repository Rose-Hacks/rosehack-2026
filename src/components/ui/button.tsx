...
const Button = (props: { text: string; link: string }) => {
  return (
    <Link
      href={props.link}
      target="_blank"
      className="py-.5 flex items-center justify-center rounded-xl border-4 border-white/30 bg-rosehack-yellow px-10 text-xl text-gray-800 duration-150 hover:cursor-pointer hover:border-white/40 hover:bg-rosehack-yellow/80"
    >
      {props.text}
    </Link>
  );
